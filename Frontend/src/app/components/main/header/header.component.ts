...
export class HeaderComponent implements OnInit, OnDestroy {
  ...
  userAvatar: string = ''; // Store avatar initials
  userEmail :string ='';
 ...
 defaultNavItems: NavItem[] = [
  ...
  ngOnInit(): void {
    ...
    this.authService.authState$.pipe(takeUntil(this.destroy$)).subscribe(() => {
     this.isLoggedIn = this.authService.isAuthenticated();
      ...
      this.userEmail = this.authService.getUserData().user.email ||'';
      this.userAvatar = this.generateAvatar(this.userName);
      console.log("Username:", this.userName);
      console.log("Avatar Text:", this.userAvatar);

       // Filter navItems dynamically
    this.navItems = this.isLoggedIn
      ? [...this.defaultNavItems] // Show general items only
      : [...this.defaultNavItems, this.registerNavItem]; // Add Register when logged out
       console.log("Updated Nav Items:", this.navItems);
    })
  }
  ...
  const nameParts = name.trim().split(/\s+/); // Handles multiple spaces
  console.log("Name Parts:", nameParts);

  return nameParts.length > 1
    ? nameParts[0][0].toUpperCase() + nameParts[1][0].toUpperCase() // "John Doe" → "JD"
    : nameParts[0][0].toUpperCase(); // Handles single-word names
}
  ...
  onEscapeKey(): void {
    ...
    return nameParts.length > 1
      ? nameParts[0][0] + nameParts[1][0] // Example: "Aster Park" -> "AP"
    ...
    try {
      ...
      this.authService.signOut()
       this.isLoggedIn = false;
         ...
         // Reset navigation items for logged-out state
